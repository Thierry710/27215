<<<<<<< HEAD
# OOP Assignment – Kamanzi Ishimwe Thierry (27215)
...
- Every console output must include the student ID. This is enforced centrally via `_27215.StudentOutput`, which wraps `System.out` so no manual string changes are needed.

## Package Layout
- `_27215/StudentOutput.java` – shared utility that appends `27215` to every `System.out` call once `StudentOutput.enable()` runs. All `main` methods enable it at startup.
- `_27215/q1` … `_27215/q10` – per-question modules. Typical contents:
...
## Run Examples
Because the root package is `_27215`, every `java` command must include that namespace.

### Interactive runner (example: Question 1 Hospital System)
```powershell

java _27215.q1.HospitalManagementSystem
```
### Automated test harness (example: Question 1 tests)
```powershell

java _27215.q1.TestHospitalSystem
```
Replace `q1` with `q2` … `q10` and the class name (`SchoolManagementSystem`, `TestSchoolSystem`, etc.) to run any other scenario. All runners automatically call `StudentOutput.enable()`, so outputs are already suffixed with `27215`.
...
# 27215
OOP Assignment 
>>>>>>> aafa2b1d65794788d396b4510acda9bceda3c551
